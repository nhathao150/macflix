...
import { ListVideo, CircleAlert, MoreHorizontal, ChevronUp, Mic2, ChevronLeft, ChevronRight } from 'lucide-react'; 
import Link from 'next/link';

// Số tập mỗi Tab và số tập hiển thị mặc định
...
export default function MovieDetailPage() {
  ...
  // Player Engine (HLS & Apple Native)
  useEffect(() => {
  ...
  return (
    <main className="min-h-screen bg-[#050505] text-white selection:bg-cyan-500/30 pb-20 relative overflow-hidden">
      ...
      <div className="max-w-[1400px] mx-auto px-4 md:px-8 pt-[80px] md:pt-[100px] relative z-10 flex flex-col gap-8">
        ...
        {/* 2. THÔNG TIN CƠ BẢN (TIÊU ĐỀ PHIM ĐƯỢC ĐƯA LÊN ĐÂY) */}
        ...
        {/* 3. CONTAINER CHỌN SERVER & DANH SÁCH TẬP */}
            ...
            {episodesList.length > 1 && (
                <div>
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
                        ...
                        {/* THANH CUỘN CỤM TẬP (Có nút lướt và tên linh hoạt) */}
                        {episodeGroups.length > 1 && (
                            <div className="relative group/tabs flex-1 overflow-hidden flex items-center w-full md:max-w-[75%] lg:max-w-[80%]">
                                ...
                                <button
                                    ...
                                    className="absolute left-0 z-10 w-8 h-8 flex items-center justify-center bg-[#141414]/90 hover:bg-[#2a2a2a] backdrop-blur-md border border-white/10 rounded-full transition-all opacity-0 group-hover/tabs:opacity-100 hidden md:flex"
                                >
                                ...
                                <div 
                                    ref={tabContainerRef}
                                    className="flex gap-2 overflow-x-auto scrollbar-hide bg-black/30 p-1 rounded-xl w-full px-2 md:px-8 snap-x"
                                >
                                    {episodeGroups.map((group, idx) => {
                                        ...
                                        return (
                                            <button
                                                ...
                                                className={`shrink-0 px-4 py-1.5 rounded-lg text-xs font-bold transition-all snap-start ${
                                                    isActive 
                                ...
                                <button
                                    ...
                                    className="absolute right-0 z-10 w-8 h-8 flex items-center justify-center bg-[#141414]/90 hover:bg-[#2a2a2a] backdrop-blur-md border border-white/10 rounded-full transition-all opacity-0 group-hover/tabs:opacity-100 hidden md:flex"
                                >
                    ...
                    <div className="flex flex-wrap gap-2 md:gap-3">
                        {visibleEpisodes.map((ep: any, localIndex: number) => {
                            ...
                            return (
                                <button
                                    key={ep.slug}
                                    onClick={() => {
                                        setCurrentEpisodeIndex(globalIndex);
                                        window.scrollTo({ top: 0, behavior: 'smooth' });
                                    }}
                                    className={`w-[calc(25%-6px)] sm:w-[calc(20%-8px)] md:w-[calc(10%-11px)] py-2 md:py-3 rounded-xl text-xs font-bold transition-all border ${
                                        isPlaying 
                                        ? 'bg-gradient-to-tr from-cyan-500 to-blue-600 text-white border-transparent shadow-[0_0_15px_rgba(6,182,212,0.4)] scale-105 z-10' 
                                        : 'bg-[#1a1a1c] text-white/70 border-white/5 hover:bg-white/20 hover:text-white hover:border-white/20'
                                    }`}
                                >
                                    {ep.name.replace('Tập ', '')}
                                </button>
                        ...
                        {hasMoreInGroup && !isExpanded && (
                            <button
                                ...
                                className="w-[calc(25%-6px)] sm:w-[calc(20%-8px)] md:w-[calc(10%-11px)] py-2 md:py-3 rounded-xl text-xs font-bold transition-all border bg-white/5 text-white/70 border-white/10 hover:bg-white hover:text-black flex items-center justify-center"
                            >
                    ...
                    {hasMoreInGroup && isExpanded && (
                        <div className="mt-4 flex justify-center">
                            <button
                                ...
                                className="flex items-center gap-1 text-xs font-bold text-white/50 hover:text-white uppercase tracking-widest transition-colors py-2 px-4 rounded-full hover:bg-white/10"
                            >
        ...
        {/* 4. MÔ TẢ PHIM */}
         <div 
            className="text-white/80 text-sm leading-relaxed p-4 md:p-6 rounded-2xl bg-[#141414] border border-white/5 shadow-inner"
                ...
                .map((name, idx) => {
                    ...
                    return (
                        <div key={idx} className="flex items-center gap-3 bg-[#141414] border border-white/5 pr-4 rounded-full hover:bg-white/10 transition-colors cursor-pointer group">
                            <div className={`w-10 h-10 rounded-full bg-gradient-to-tr ${randomColor} flex items-center justify-center text-white font-bold text-sm shadow-sm group-hover:scale-105 transition-transform`}>
                                {initials}
                            </div>
                            <div>
