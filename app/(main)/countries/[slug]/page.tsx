...
import { getMoviesByCountryPaginated, getMoviesByCountryAndGenre } from '@/services/movie.service';
...
function CountryContent() {
  ...
  const handleApplyFilters = () => {
    ...
    if (tempCountry && tempCountry !== slug) {
      router.push(`/countries/${tempCountry}?${newParams.toString()}`);
    } else {
      router.push(`/countries/${slug}?${newParams.toString()}`);
    }
  ...
  const handleResetFilters = () => {
    ...
    setTempGenre('');
    setTempYear('');
    router.push(`/countries/${slug}?page=1`);
    setIsFilterDrawerOpen(false);
  };
