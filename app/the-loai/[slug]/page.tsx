...
function GenreContent() {
  ...
  useEffect(() => {
    const fetchMovies = async () => {
      setIsLoading(true);
      const data = await getMoviesByGenrePaginated(slug, currentPage, 64);
  ...
  const handlePageChange = (newPage: number) => {
    ...
    router.push(`/the-loai/${slug}?page=${newPage}`);
  };
    ...
    <main className="min-h-screen bg-[#010030] text-white selection:bg-[#F042FF]/30 pb-20">
      ...
      <div className="max-w-[1800px] mx-auto px-4 md:px-8 pt-[120px]">
        ...
        {isLoading ? (
          ...
          <div className="text-center text-white/50 mt-20">Không có phim nào trong thể loại này.</div>
        )}
      </div>
    </main>
  );
